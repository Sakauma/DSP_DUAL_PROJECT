...
`docs/INTERFACE_AND_DATA_STRUCTURES.md` 整合了硬件接口与数据结构约束。SRIO 相关重点包括：
...
预处理 SRIO 转视频处理接口相关重点包括 BRAM 行状态、16-bit BRAM 数据读取、64-bit AXIS 视频输出、SRIO AXIS 接口和 DDR AXI4 读接口。算法侧只消费硬件接口层转换后的图像视图，不直接绑定这些硬件信号。
...
1. 阅读 `docs/INTERFACE_AND_DATA_STRUCTURES.md`，明确 SRIO API、EB4110 图像输出、预处理视频接口和算法侧使用边界。
2. 重点确认 WDMA 通道0限制、SelfID 全局共享、图像尺寸、参数行、帧计数、ping-pong 地址和 DSP1/DSP2 ID。
